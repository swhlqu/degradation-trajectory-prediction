...
class Dataset_generation(Dataset):
    ...
    def load_npy(self):
        self.x_train = np.load("x_train_preds.npy").reshape(-1, 3000, 1)  # (N, 10)
        self.x_valid = np.load("x_valid_preds.npy").reshape(-1, 3000, 1)
        self.x_test = np.load("x_test_preds.npy").reshape(-1, 3000, 1)
        self.y_train = np.load("y_train_preds.npy")
        self.y_valid = np.load("y_valid_preds.npy")
        self.y_test = np.load("y_test_preds.npy")
...
def adjust_learning_rate(optimizer, epoch, learning_rate, lradj):
    if lradj == 'type1':
        ...
        lr_adjust = {
            ...
            5000: 5e-6, 6000: 1e-6, 7000: 5e-7, 8000: 1e-7, 9000: 5e-8
        }
...
class Exp():
    def __init__(self, enc_in, c_out, d_model, n_heads, e_layers,
        ...
        self.checkpoints_path = './checkpoint/Attention'
    ...
    def deal_length(self, preds, trues):
        total_preds = pd.DataFrame(np.zeros(shape=(10, 1)))
        total_trues = pd.DataFrame(np.zeros(shape=(10, 1)))
        for i in range(trues.shape[1]):
            each_trues = trues[:, i].reshape(-1, 1)
            each_preds = preds[:, i].reshape(-1, 1)
            if np.where(each_trues == 0)[0].size > 0:
                index = np.where(each_trues == 0)[0][0]
                new_trues = each_trues[:index, :].reshape(-1, 1)
                new_preds = each_preds[:index, :].reshape(-1, 1)
            else:
                new_trues = each_trues
                new_preds = each_preds
            total_preds = pd.concat([total_preds, pd.DataFrame(new_preds[:-1, :])], axis=1, ignore_index=True)
            total_trues = pd.concat([total_trues, pd.DataFrame(new_trues[:-1, :])], axis=1, ignore_index=True)
        return total_preds.iloc[:, 1:], total_trues.iloc[:, 1:]
    ...
    def predict(self, test_loader):
        ...
        preds = preds.reshape(-1, 3000)
        preds = preds
        trues = trues.reshape(-1, 3000)
        trues = trues

        preds_last = np.array(preds_last).reshape(-1, 3000)
        trues_last = np.array(trues_last).reshape(-1, 3000)
        ...
        new_preds.to_csv('preds.csv', header=False, index=False)
        new_trues.to_csv('trues.csv', header=False, index=False)
...
exp = Exp(enc_in=1, c_out=1,
          d_model=32, n_heads=4, e_layers=3, d_ff=32 * 4,
          dropout=0.05, activation='gelu',
          distil=True, patience=5000)
print('>>>>>>>start training>>>>>>>>>>>>>>>>>>>>>>>>>>')
exp.train(train_loader, vali_loader, test_loader, train_epochs=10000)
torch.cuda.empty_cache()
print('>>>>>>>start testing>>>>>>>>>>>>>>>>>>>>>>>>>>')
exp.predict(vali_loader)
