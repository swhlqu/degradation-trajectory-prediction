...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.linear_layer1 = nn.Linear(10, 64)
...
class Exp():
    ...
    def vali(self, vali_loader, criterion):
        ...
        for i, (batch_enc, batch_output) in enumerate(vali_loader):
            mu_prediction, batch_output = self._process_one_batch(batch_enc, batch_output)
            loss = criterion(mu_prediction.detach().cpu(), batch_output.detach().cpu())
    ...
    def train(self, train_loader, vali_loader, test_loader, train_epochs):
        ...
        for epoch in range(train_epochs):
            ...
            for i, (batch_enc, batch_output) in enumerate(train_loader):
                ...
                mu_prediction, batch_output = self._process_one_batch(batch_enc, batch_output)
                loss = criterion(mu_prediction, batch_output)
    ...
    def predict(self, test_loader):
        ...
        for i, (batch_enc, batch_output) in enumerate(test_loader):
            mu_prediction, batch_output = self._process_one_batch(batch_enc, batch_output)
            preds.append(mu_prediction.detach().cpu().numpy())
            trues.append(batch_output.detach().cpu().numpy())

        preds_last = preds.pop(-1)
        trues_last = trues.pop(-1)

        preds_last = preds_last.reshape(-1, 10)
        trues_last = trues_last.reshape(-1, 10)
        ...
        pd.DataFrame(preds).to_csv('preds.csv', header=False, index=False)
        pd.DataFrame(trues).to_csv('trues.csv', header=False, index=False)
    ...
    def _process_one_batch(self, batch_enc, batch_output):
        ...
        mu_prediction = self.model(batch_enc)

        return mu_prediction, batch_output
...
exp = Exp(patience=4000)
print('>>>>>>>start training>>>>>>>>>>>>>>>>>>>>>>>>>>')
exp.train(train, valid, test, train_epochs=10000)
torch.cuda.empty_cache()
print('>>>>>>>start testing>>>>>>>>>>>>>>>>>>>>>>>>>>')
exp.predict(test_loader=test)
